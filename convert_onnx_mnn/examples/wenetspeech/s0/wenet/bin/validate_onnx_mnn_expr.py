...
def val_encoder(asr_model, args):
    ...
    mnn_required_cache_size = to_numpy(required_cache_size)
    ...
    mnn_input_chunk = vars["chunk"]
    mnn_input_offset = vars['offset']
    mnn_input_required_cache_size = vars['required_cache_size']
    mnn_input_att_cache = vars['att_cache']
    mnn_input_cnn_cache = vars['cnn_cache']

    if mnn_input_chunk.data_format == F.NC4HW4:
        mnn_input_chunk.reorder(F.NCHW)
    mnn_input_chunk.write(mnn_chunk.tolist())
    mnn_input_offset.write(mnn_offset.tolist())
    mnn_input_required_cache_size.write(mnn_required_cache_size.tolist())
    mnn_input_att_cache.write(mnn_att_cache.tolist())
    mnn_input_cnn_cache.write(mnn_cnn_cache.tolist())

    mnn_output_0 = vars['output']
    #print(mnn_output_0.data_format)
    #print(mnn_output_0)
    assert 0==1
    mnn_output_1 = vars['r_att_cache']
    mnn_output_2 = vars['r_cnn_cache']
