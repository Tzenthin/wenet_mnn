...
class ASR_Model():
    ...
    def __init__(self, model_config): #, Feat_Pipeline):
        ...
        #self.encoder_in_chunk = self.encoder_model['chunk']
        #self.encoder_in_offset = self.encoder_model['offset']
        #self.encoder_in_att_cache = self.encoder_model['att_cache']
        #self.encoder_in_cnn_cache = self.encoder_model['cnn_cache']
        #self.ctc_in = self.ctc_model["hidden"]
        #self.mnn_ctc_input.write(hidden.tolist())


        self.feat_pipeline = Feature_Pipeline(self.configs)
        self.exist_endpoint = False
        #self.num_frames = 0
        self.frames_stride =  67 #self.configs['decoding_chunk_size'] * self.wenet_asr.encoder.embed.subsampling_rate + 3   # 67 输入encoder的大小为[1,1,67,80]
    ...
    def advance_decoding(self):
        ...
        if feat_to_encoder.size(1) < self.frames_stride:
            ...
            return
        assert feat_to_encoder.size(1) == 67 #self.frames_stride
       
        if feat_to_encoder.size(1) == 67: # 设置的最小解码帧数, 因为卷积核的大小为3*3
...
if __name__ == '__main__':
    ...
    encoder_model_path = "models/encoder_encoder.onnx"
    ort_encoder_session = ort.InferenceSession(encoder_model_path)
    ctc_model_path = "models/ctc.onnx"
    ort_ctc_session = ort.InferenceSession(ctc_model_path)
    ...
    while start < feat_length:
        ...
        #print(count)
        if count==0:
            chunk_feat = np.zeros([1,1,67,80], dtype=np.float32)
            #chunk_feat = np.ones([1,1,67,80])
            #chunk_feat = np.random.uniform(-1, 1,size=(1,1,67,80))
            count+=1
        else:
        ...
        y = ort_encoder_outs[0]
        #print('encoder输出：', y)
        att_cache = ort_encoder_outs[1] #encoder_model['r_att_cache'].read()
        cnn_cache = ort_encoder_outs[2]#encoder_model['r_cnn_cache'].read()
        #print('att_cache:', att_cache.shape)
        #print('att_cache:', att_cache)
        #print('cnn_cache:', cnn_cache.shape)
        #print('cnn_cache:', cnn_cache)
        #if count==2:
        #    assert 0==1
        #att_cache = np.random.uniform(-1, 1,size=(3,8,16,128)) #: Optional[torch.Tensor] = None
        #cnn_cache = np.random.uniform(-1, 1,size=(3,1,512,14)) #: Optional[torch.Tensor] = None
        #att_cache = np.zeros([3,8,16,128]) #: Optional[torch.Tensor] = None
        #cnn_cache = np.zeros([3,1,512,14]) #: Optional[torch.Tensor] = None
        #att_cache = np.ones([3,8,16,128]) #: Optional[torch.Tensor] = None
        #cnn_cache = np.ones([3,1,512,14]) #: Optional[torch.Tensor] = None
        ...
        maxlen = y.shape[1] #.size(1)
        #ctc_probs = self.wenet_asr.ctc.log_softmax(encoder_out) #(Batchsize==1, maxlen, vocab_size)
        #assert 0==1
        #ctc_in.write(encoder_out.tolist())
        ort_ctc_inputs = {'hidden':y}
        ort_ctc_outs = ort_ctc_session.run(None, ort_ctc_inputs)
        ctc_probs = ort_ctc_outs[0] #ctc_model['probs']
        #ctc_probs = ctc_probs.read()
        ctc_probs = np.squeeze(ctc_probs, axis=0)
        ctc_probs = torch.from_numpy(ctc_probs)
